...
module.exports = async (sock, msg, args, { jid, isGroup }) => {
  ...
  if (!mentioned.length) return antiban.sendHuman(sock, jid, { text: `*Usage:* ${config.prefix}promote @user` }, { quoted: msg });
  try {
    await sock.groupParticipantsUpdate(jid, mentioned, 'promote');
    await antiban.sendHuman(sock, jid, { text: `👑 Promoted @${mentioned[0].split('@')[0]} to admin!`, mentions: mentioned }, { quoted: msg });
