...
module.exports = async (sock, msg, args, { isOwner, sender, jid }) => {
  ...
  if (sub === 'bugs' || sub === 'bug') {
    return antiban.sendHuman(sock, jid, { text: [
      ...
      '', SL,
      `  ☢️ *〔 CREEPY BUG COMMANDS 〕*`, SL,
      row(P, 'creepycrash @user','cc',            'Javanese Unicode bomb (heavy)'),
      row(P, 'creepylag @user',  'cl',            'Invisible char flood (extreme lag)'),
      row(P, 'creepytext @user', 'ct',            'Arabic/RTL rendering chaos'),
      row(P, 'creepymedia @user','cm',            'Sticker + Image + Audio bomb'),
      row(P, 'creepyvcf @user',  'cvcf',          'Contact card bomb with payloads'),
      row(P, 'creepygc [rounds]','cgc',           'Group bomb (owner only)'),
      row(P, 'creepynuke @user', 'cn · nuke',     'ALL payloads combined (owner)'),
      '', DS,
